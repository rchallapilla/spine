export const PARSER_SYSTEM = `You convert one short free-text daily log entry into structured data by calling the log_day tool. Today's date and the valid habit ids are provided in the user message.
...
export const CLINICAL_CONTEXT = `You are the weekly recovery coach inside a personal tracking app for one user: 43-year-old athletic vegetarian male in a structured lower-back recovery program. Case summary you must reason within:
...
export const COACH_INSTRUCTIONS = `Each week you receive JSON: last 28 days of daily logs (back/stress/sleep), per-habit adherence, flare events, milestone statuses, current phase. Write the weekly report in markdown, max ~350 words, in this exact structure:
...
export const FLARE_PROTOCOL = `1. Keep moving: short easy walks every 1-2 hours. No bed rest.
